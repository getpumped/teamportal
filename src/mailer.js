...
module.exports = { 
  ...
  sendMemberWelcome: function(user, callback) {
    ...
    fs.readFile(file, 'utf8', function(err, template) {
      if(!err) {
        var html = ejs.render(template, { filename: file,compileDebug: true, title: 'Team Pumped Registration', defaultDate: dateFormat(new Date(), "dd/mm/yyyy"), dateFormater: dateFormat,
...
function sendTeamUpdate(user, global_vars, callback) {
  ...
  ejs.render('./emails/team-update', { title: 'Team Update', logs: logs
                                      , defaultDate: dateFormat(new Date(), "dd/mm/yyyy"), dateFormater: dateFormat,
                                      errors: [], messages: [], merge_vars: merge_vars,
                                      leaderboard: global_vars.leaderboard, teamLeaderboard: global_vars.teamLeaderboard},function(err, html) {
                                        var params = { "message": { "html": html,"subject": "Team Update",
                                                                   "from_email": config.emailFrom, "from_name": config.emailFromName,
                                                                   "to": [{"email": user.email,"name": user.username}],
                                                                   "track_opens": true,  "track_clicks": true,"auto_text": true,"auto_html": false,

...
exports.teamUpdate = function(req, res) {
  if(req.session.user.teamname === null) {
    ...
    pumped.getUserLogs(req.session.user.username, { limit: 10, skip: 0, sort: 'date' }, function(err, logs) {
      if(err) {
        console.log(err);
      ...
      pumped.getLeaderboard(function(err, leaderboard) {
        ...
        pumped.getTeamLeaderboard(req.session.user.teamname, function(err, teamLeaderboard) {
          ...
          var options = { settings: {}, compileDebug: true, title: 'Team Update', logs: logs, 
                         defaultDate: dateFormat(new Date(), "dd/mm/yyyy"), dateFormater: dateFormat, errors: req.flash('errors'), 
                         messages: req.flash('messages'), merge_vars: merge_vars, leaderboard: leaderboard, teamLeaderboard: teamLeaderboard}
          ejs('views/emails/team-update.ejs', options, function(err, html) {
...
exports.memberWelcome = function(req, res) {
  ...
  var options =  { settings: {}, compileDebug: true, title: 'Team Update', 
                  defaultDate: dateFormat(new Date(), "dd/mm/yyyy"), dateFormater: dateFormat,errors: [], messages: [], merge_vars: merge_vars};
  ejs('views/emails/member-welcome.ejs', options, function(err, html) {
    var params = { "message": { "html": html,"subject": "Team Update",
                               "from_email": config.emailFrom, "from_name": config.emailFromName,
                               "to": [{"email": req.session.user.email,"name": req.session.user.username}],
                               "track_opens": true,  "track_clicks": true,"auto_text": true,"auto_html": false,
